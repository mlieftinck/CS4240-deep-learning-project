...
def preprocessing():
    ...
    x = np.linspace(1, 112, 112)
    y = np.linspace(1, 114, 114)
    z = np.linspace(1, 112, 112)
    ...
    values = interpolated_image_normalized

    sc = ax.scatter(x_flat, y_flat, z_flat, c=values.flatten(), alpha= 1)

    plt.colorbar(sc)
    plt.show()

    return interpolated_image_normalized
